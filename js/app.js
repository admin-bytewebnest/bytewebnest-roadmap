      ...
      entries.forEach((entry) => {
        ...
        links.forEach((link) => {
          link.classList.toggle(
            "is-active",
            link.getAttribute("data-topic-link") === id
          );
        });
...
function updateProgress() {
  ...
  const modulePercent = currentTopicsTotal
    ? Math.round((currentCompleted / currentTopicsTotal) * 100)
    : 0;
  ...
  const coursePercent = totalCourseTopics
    ? Math.round((completedCourseTopics / totalCourseTopics) * 100)
    : 0;
